...
- 超过30%后网络实际不可用（不适合防御评估）

**预期结果**：
- 无防御：Avg Attack ≈ 100%（所有丢包率）
- 滚动计数器：Avg Legit 随丢包率显著下降
- 滑动窗口：Avg Legit 下降较为平缓
- 挑战-响应：Avg Legit 受通信开销轻微影响
...
**预期结果**（关键发现）：
- 滚动计数器：Avg Legit 急剧下降（乱序包被拒绝为重放）
  - 0%乱序：100%可用性
  - 30%乱序：~40-60%可用性
- 滑动窗口：Avg Legit 保持稳定（设计用于容忍乱序）
  - 0-30%乱序：保持90-100%可用性
- 挑战-响应：由于通信开销轻微下降

*本实验揭示了严格基于计数器的防御在真实网络中的根本局限性。*
...
**预期结果**：
- Avg Legit：随窗口大小增加而上升（W=1: ~80%, W=5: ~95%, W=20: ~98%）
- Avg Attack：随窗口大小增加而上升（W=1: ~2%, W=5: ~5%, W=20: ~15%）
- 最优平衡：W = 5-7（可用性 >95%，攻击率 <10%）
